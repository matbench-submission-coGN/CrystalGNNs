...
model_default = {
    "name": "Schnet",
    "inputs": [{"shape": (None,), "name": "node_attributes", "dtype": "int32", "ragged": True},
               {"shape": (None,), "name": "edge_distance", "dtype": "float32", "ragged": True},
               {"shape": (None, 2), "name": "edge_indices", "dtype": "int64", "ragged": True},
               {"shape": (None,), "name": "multiplicities", "dtype": "int32", "ragged": True}],
    "input_embedding": {'node_size': 128,
                'edge_size': 128,
                'atomic_mass': True,
                'atomic_radius': True,
...
def make_model(inputs: list = None,
    ...
    edge_distance = ks.layers.Input(**inputs[1])
    edge_index_input = ks.layers.Input(**inputs[2])
    multiplicities_ = ks.layers.Input(**inputs[3])
    multiplicities = tf.cast(tf.expand_dims(multiplicities_, -1), tf.float32)
    
    edi = edge_index_input
    
    embedding_block = GraphNetworkConfigurator.get_input_block(**input_embedding)
    ed, n, _, _ = embedding_block([edge_distance, node_input, None, edi])
    ...
    if output_embedding == 'graph':
        out = MultiplcityReadout(**node_pooling_args)([n, multiplicities])
        if use_output_mlp:
    ...
    model = ks.models.Model(inputs=[node_input, edge_distance, edge_index_input, multiplicities_], outputs=out)
    return model
